...
public class InputFormTests extends BaseTest{
    ...
    public void verifyTitle()
	{
        ...
        extentTestThread.get().log(Status.INFO,"Starting test to verify title");
 
        String title ="Selenium Grid Online | Run Selenium Test On Cloud";
        Assert.assertEquals(title,driver.getTitle());
        extentTestThread.get().log(Status.INFO,"Verified the expected page title as : " + title);
    ...
    public void singleInputTest() 
	{
		...
		extentTestThread.set(extentTest);
		extentTestThread.get().log(Status.INFO,"Starting test to verify single input");
		
		//Clicks on the simple form demo option in the selenium playground
		extentTestThread.get().log(Status.INFO,"Clicking on Simple Form Demo");
        driver.findElement(By.xpath("//*[text()='Simple Form Demo']")).click();
            
        //Enter the message in the enter message input box
        String expectedMessage = "Hello World";
        extentTestThread.get().log(Status.INFO,"Entering the message as : " + expectedMessage);
        driver.findElement(By.id("user-message")).sendKeys(expectedMessage);
            
        //Click on Get checked value button
        extentTestThread.get().log(Status.INFO,"Clicking on Get checked value");
    ...
    public void multipleInputTest() 
	{
		...
		extentTestThread.set(extentTest);
		extentTestThread.get().log(Status.INFO,"Starting test to verify multiple input");
        ...
        extentTestThread.get().log(Status.INFO,"Entering the values as 5 and 10");
        driver.findElement(By.id("sum1")).sendKeys("5");
        driver.findElement(By.id("sum2")).sendKeys("10");
        
        //Click on the Get Sum button
        extentTestThread.get().log(Status.INFO,"Clicking on Get Sum");
...
//        Assert.assertEquals(actualSum,"15", "Expected and actual values do not match.");
        ...
        extentTestThread.get().log(Status.INFO,"Verified the expected sum as : 15");
